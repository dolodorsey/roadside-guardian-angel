...
const LiveRescue: React.FC<Props> = ({ jobId, onComplete, onClose }) => {
  ...
  const statusBg=isSearching?'bg-amber-500':isActive?'bg-blue-500':isDone?'bg-green-500':'bg-gray-400';
      ...
      <div className="flex-1 relative">
        ...
        <div className="absolute top-safe left-0 right-0 px-5 mt-4 flex justify-between z-[1000]">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={()=>jobSafety(jobId,'safe_mode',{enabled:true}).catch(()=>{})} className="w-11 h-11 rounded-2xl bg-white border border-gray-200 shadow-card flex items-center justify-center"><Shield className="w-[18px] h-[18px] text-orange-500"/></button>
          </div>
        </div>
      </div>
